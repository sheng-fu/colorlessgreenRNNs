...
models = c('1M', '2M', '4M', '8M', '16M', '32M', '64M', '60')
...
overall$epoch = gsub("M", "", overall$epoch)
...
broad_breakdown$epoch = as.factor(as.numeric(gsub('epoch ', '', broad_breakdown$epoch)))
...
simple_lm1 = mutate(simple_lm, label = if_else(epoch == '82', 
            as.character(linguistics_term), NA_character_))

p = ggplot(simple_lm1, aes(x = epoch, y = m_pred, group = linguistics_term, colour = linguistics_term)) + 
  geom_line(aes(color=linguistics_term), size = 1.2)+ xlim(0,9) +
  geom_point(aes(color=linguistics_term), size = 2)+ xlab("training size (M)") +
  ...
  theme(
    ...
    axis.text.x = element_text(size = 14),
    axis.title.y = element_text(size = 16)) +
  geom_label_repel(aes(label = label), direction = "y", nudge_x = 3, na.rm = TRUE, hjust = 0, cex = 6)
